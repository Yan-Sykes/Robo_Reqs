"""GUI (PyQt6) para execução e monitoramento do SAP Robot.

Este módulo provê a interface gráfica para iniciar o processo de atualização de
requisições no SAP, acompanhar o progresso, exibir logs e gerenciar seleção de
arquivo Excel. A execução pesada ocorre em uma thread separada (WorkerThread),
que se comunica com a camada de automação (Reqs.py) via callbacks.
"""
...
# Integração com a camada de automação SAP
import Reqs

# Identificação do usuário logado (utilizado em logs locais)
USUARIO = getpass.getuser().upper()

# Paleta de cores corporativas da interface
BG_COLOR = '#82298c'
FG_COLOR = "#ffffff"
ACCENT_COLOR = '#a94bb9'
LOG_BG_COLOR = '#682077'
...
# Diretório de logs locais do front-end (por execução)
LOGS_DIR = os.path.join(os.path.expanduser("~"), "SAP_Robo_Logs")
if not os.path.exists(LOGS_DIR):
    os.makedirs(LOGS_DIR)

# Caminho do arquivo de configuração persistente da GUI
CONFIG_FILE = os.path.join(LOGS_DIR, "config.json")

def load_config():
    """Carrega as configurações persistentes da interface.

    Retorna um dicionário com preferências do usuário, como o último arquivo
    Excel selecionado. Caso o arquivo esteja ausente ou inválido, retorna
    um dicionário vazio.
    """
...
def save_config(config):
    """Persiste as configurações da interface em disco.

    Parâmetros:
        config: dict de configurações a serem salvas (JSON).
    """
    try:
...
class CleanCard(QFrame):
    """Container visual com estilo de cartão (card) e opção de elevação."""
...
class ModernButton(QPushButton):
    """Botão com estilos predefinidos (primary, secondary, ghost)."""

    def __init__(self, text, style: str = 'primary', icon_only: bool = False):
        super().__init__(text)
    ...
    def _setup_style(self):
        """Aplica a folha de estilos (CSS Qt) de acordo com o tipo de botão."""
...
class StatusDot(QLabel):
    """Indicador visual de status operacional (idle, running, success, error, warning)."""
    ...
    def set_status(self, status: str):
        """Atualiza o estado e reflete a cor correspondente."""
        self.status = status
        self._update_style()
...
class CleanProgressBar(QProgressBar):
    """Barra de progresso minimalista com tema corporativo."""
...
class WorkerThread(QThread):
    """Thread dedicada à execução do processo de automação SAP.

    Responsável por acionar a camada de automação (Reqs.main), repassar logs e
    progresso para a interface e registrar logs locais por execução.
    """
    ...
    def stop(self):
        """Sinaliza a interrupção da execução de longa duração."""
        self._running = False

    def run(self):
        """Método principal da thread: configura callbacks e executa o robô."""
        start_time = time.time()
        self._write_log(f"=== Execução iniciada em {time.strftime('%d/%m/%Y %H:%M:%S')} ===")

        # Encaminhar callbacks para o módulo de automação
        Reqs.set_callbacks(
        ...
        # Definir o arquivo Excel a ser usado (quando selecionado via GUI)
        if self.arquivo_excel:
            Reqs.set_arquivo_excel(self.arquivo_excel)
            self._write_log(f"Arquivo selecionado: {self.arquivo_excel}")

        try:
            # Executar o processo principal de automação
            Reqs.main()
    ...
    def _write_log(self, texto: str):
        """Registra mensagens em arquivo de log local da execução atual."""
        try:
...
class MainWindow(QWidget):
    ...
    Oferece seleção de arquivo Excel, exibição de logs, indicação de status
    e controle de execução (iniciar/cancelar) do robô SAP.
    """
    ...
    def __init__(self):
        ...
        # Carrega configurações persistentes (ex.: último arquivo)
        ...
        # Exibe o arquivo salvo (se existente)
    ...
    def _setup_window(self):
        """Configura propriedades da janela (ícone e estilo global)."""
        # Ícone da janela via URL (fallback simples se falhar)
        ...
        # Estilo global (tematização da UI)
        style_sheet = f"""
            * {{
    ...
    def _setup_ui(self):
        """Monta a hierarquia de widgets e layouts da janela principal."""
        ...
        subtitle = QLabel("Automação de alteração de Requisições")
    ...
    def _setup_logo(self, logo_label: QLabel):
        """Tenta carregar o logotipo a partir de URL. Define fallback se indisponível."""
    ...
    def _set_fallback_logo(self, logo_label: QLabel):
        """Define um ícone textual simples como fallback de logotipo."""
        logo_label.setText("●")
        logo_label.setFont(QFont("Inter", 24))
        logo_label.setStyleSheet(f"color: {COLORS['accent']};")

    def selecionar_arquivo(self):
        """Abre o diálogo para seleção de arquivo Excel a ser processado."""
        # Diretório inicial: último utilizado ou pasta do usuário
    ...
    def limpar_arquivo(self):
        """Remove a seleção de arquivo e reverte ao caminho padrão (Reqs.py)."""
        ...
        # Remover configuração persistida
        if 'ultimo_arquivo' in self.config:
            del self.config['ultimo_arquivo']
            save_config(self.config)

    def iniciar_execucao(self):
        """Valida a seleção e inicia a execução em thread dedicada."""
        if self.worker and self.worker.isRunning():
            return
        
        # Validação do arquivo selecionado (quando aplicável)
        ...
        # Criação e ligação de sinais da thread
    ...
    def adicionar_log(self, texto: str):
        """Adiciona uma linha ao console de logs com timestamp automático."""
        # Evita duplicação de timestamp caso já venha formatado
    ...
    def atualizar_progresso(self, valor: int):
        """Atualiza visualmente a barra e o texto de progresso."""
        self.progress_bar.setValue(valor)
        self.progress_value.setText(f"{valor}%")

    def atualizar_status(self, texto: str, tipo: str):
        """Atualiza o indicador de status e a descrição correspondente."""
        self.status_dot.set_status(tipo)
        self.status_text.setText(texto)

    def cancelar_execucao(self):
        """Solicita a interrupção da execução e restaura o estado da UI."""
        if self.worker and self.worker.isRunning():
            self.worker.stop()
            self.worker.quit()
            self.worker.wait()
    ...
    def execucao_finalizada(self, tempo_total: float):
        """Callback de finalização: apresenta resumo e restaura a UI."""
        # Restaurar estado dos controles
        self.cancel_btn.setEnabled(False)
        self.start_btn.setEnabled(True)
        self.start_btn.setText("Executar")
        ...
        # Determina o tipo de mensagem com base no status atual
        status_atual = self.status_text.text().lower()
        
        # Caixa de diálogo de conclusão
    ...
    def closeEvent(self, event):
        """Garante desligamento limpo caso a execução ainda esteja em andamento."""
        if self.worker and self.worker.isRunning():
